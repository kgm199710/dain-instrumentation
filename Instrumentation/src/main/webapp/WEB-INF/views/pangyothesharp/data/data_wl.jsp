	...
	<script>
		window.onload = function() {
			//최초 접속시or클릭했던 ipi센서항목에 대한 표시값들 show=보여주기, active=흰색처리
                document.getElementById('collapseW').className += ' show'
			...
			<main>
				<div class="py-3 px-3 my-3 mx-3" style="width:1000px;">
					<div class="text-center my-3">
						<h2>지하수위계 #<%=selectId.substring(selectId.length()-2,selectId.length())%><br></h2>
					</div>
					<div class="my-3">
						현장명 : <%=pageKorName %><br>
						검색기간 : <%=start %> ~ <%=end %><br>
...
<%-- 					%> --%>
<%-- 					<%	for(int i=0; i<wlDatas.length; i++){ %> --%>
<!-- 						<tr> -->
<%-- 					<%		for(int j=0; j<wlDatas[i].length; j++){ %> --%>
<%-- 							<td><%=(wlDatas[i][j]!=null)?wlDatas[i][j]:"" %></td> --%>
<%-- 					<%		}%> --%>
<!-- 						</tr> -->
<%-- 					<% 	} %> --%>
<%-- 					<% }else{ %> --%>
...
<%-- 					<% } %> --%>
    ...
    <script type= "text/javascript">
        ...
        function drawChart() {
            ...
            data.addRows([<% for(int i=wlDatas.length-1; i>1; i--){
            		String d = wlDatas[i][0];
            		String date = d.substring(0,4) + ", " + d.substring(5,7) + ", " + d.substring(8,10) + ", " + d.substring(11,13) + ", " + d.substring(14,16) + ", " + d.substring(17,19);
            		System.out.print(date);%>
            	[new Date(<%= date%>), <%=wlDatas[i][2]%>], <% } %>
            ]);
            ...
            var options = {
            	...
            	hAxis:{
   				...
   				vAxis:{
   					...
   					<% if(manageData != null && !manageData.isEmpty()){
   						String[] limit = manageData.split(",");%>viewWindow:{min:<%=limit[0]%>,max:<%=limit[1]%>},<%}%>
   					textStyle:{color:'#000',fontName:'Arial',fontSize:10}
   					},
			series:{0:{color:'blue',lineWidth:1}}
			};
...
// 				minDate: new Date( (연도),(월) - 1,(일) ), //검색가능 최소 일자(최초데이터값일자)
	...
	<script type="text/javascript">
		function excelDown() { 
	        ...
	        html += ' <tr><td colspan="4">센서명 : 지하수위계 #<%=selectId.substring(selectId.length()-2,selectId.length())%></td></tr>';
	        html += ' <tr><td colspan="4">검색기간 : <%=start%> ~  <%=end %></td></tr>';
			html += ' <tr>';
				<% 		for(int j=0; j<wlDatas[0].length; j++){%>
			html += 	'<td><%= (wlDatas[0][j]!=null)?wlDatas[0][j]:"" %></td>';
				<%		} %>
			html += 	' </tr>';
				<%	for(int i=wlDatas.length-1; i>0; i--){ %>
			html += ' <tr>';
				<% 		for(int j=0; j<wlDatas[i].length; j++){%>
			html += 	'<td><%= (wlDatas[i][j]!=null)?wlDatas[i][j]:"" %></td>';
				<%		} %>
			html += 	' </tr>';
				<% 	} %>
			html += ' </table>';
	        ...
	        if ((ua.indexOf("MSIE ") > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) && window.navigator.msSaveBlob) { 
	            ...
	            anchor.download = '<%="지하수위계 #" + selectId.substring(selectId.length()-2,selectId.length()) + "_" + start + "_" + end%>.xls'; 
