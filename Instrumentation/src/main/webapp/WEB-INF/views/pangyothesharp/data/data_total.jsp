...
<!-- ~~ 여기까지 -->
	...
	String[][] systemDatas = (String[][])request.getAttribute("systemDatas");
	%>
<!--개별시스템전용 페이지-->
...
<head>
	...
	<script>
		window.onload = function() {
	        document.getElementById('collapseS').className += ' show'
		...
		<div id="layoutSidenav_nav">
			<nav class="sb-sidenav accordion sb-sidenav-dark" id="sidenavAccordion">
				<div class="sb-sidenav-menu">
					<div class="nav">
						<form method='GET' action= 'data_total' class="d-md-inline-block form-inline ml-3 mr-3 mr-md-3 my-0 my-md-0">
						...
						<div class="collapse" id="collapseIPI" aria-labelledby="headingTwo" data-parent="#sidenavAccordion" style="width:190px;">
						    <nav class="sb-sidenav-menu-nested nav accordion" id="sidenavAccordionPages">
						        ...
						        <div class="collapse" id="IPI-2" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                <a class="nav-link my-0 py-1" id="IPI-2_chart" href="data_all_ipi?id=0&&end=<%=end%>&time=">누적그래프</a>
						        ...
						        <div class="collapse" id="IPI-3" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                <a class="nav-link my-0 py-1" id="IPI-3_chart" href="data_all_ipi?id=1&end=<%=end%>&time=">누적그래프</a>
						        ...
						        <div class="collapse" id="IPI-7" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                <a class="nav-link my-0 py-1" id="IPI-7_chart" href="data_all_ipi?id=2&end=<%=end%>&time=">누적그래프</a>
						        ...
						        <div class="collapse" id="IPI-11" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                <a class="nav-link my-0 py-1" id="IPI-11_chart" href="data_all_ipi?id=3&end=<%=end%>&time=">누적그래프</a>
						        ...
						        <div class="collapse" id="IPI-13" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                <a class="nav-link my-0 py-1" id="IPI-13_chart" href="data_all_ipi?id=4&end=<%=end%>&time=">누적그래프</a>
		...
		<div id="layoutSidenav_content">
			<main>
				<div class="py-3 px-3 my-3 mx-3">
                        ...
                        <table>
						<%	for(int i=0; i<systemDatas.length; i++){ %>
							<tr>
						<% 		for(int j=0; j<systemDatas[i].length; j++){%>
								<td><%= systemDatas[i][j] %></td>
  						<%		} %>
  						<%		if(i==0){ %>
								<td>비고</td>
						<%		} else{%>
...
// 				minDate: new Date( (연도),(월) - 1,(일) ), //검색가능 최소 일자(최초데이터값일자)
	...
	<script type="text/javascript">
		function excelDown() { 
	        ...
	        html += ' <x:Name><%= pageName + "_" + start + "_" + end%></x:Name>'; // 시트이름 
	        ...
	        html += ' <tr>';
				<% 		for(int j=0; j<systemDatas[0].length; j++){%>
			html += 	'<td><%= systemDatas[0][j] %></td>'; <%		} %>
			html += 	' <td>비고</td>';
			html += ' </tr>';
				<%	for(int i=systemDatas.length-1; i>0; i--){ %>
			html += ' <tr>';
				<% 		for(int j=0; j<systemDatas[i].length; j++){%>
			html += 	'<td><%= systemDatas[i][j] %></td>';
					<%		} %>
					<%		if(i==0){ %>
			html += 	' <td>비고</td>';
				<%		} else{%>
	        ...
	        if ((ua.indexOf("MSIE ") > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) && window.navigator.msSaveBlob) { 
	            ...
	            anchor.download = '<%= pageName + "_" + start + "_" + end%>.xls'; 
