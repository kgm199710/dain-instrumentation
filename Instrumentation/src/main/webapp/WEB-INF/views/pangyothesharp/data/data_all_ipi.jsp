					...
					<div class="nav">
						...
						<div class="collapse" id="collapseW" aria-labelledby="headingOne" data-parent="#sidenavAccordion">
						    ...
						    <% for(int i=0; i<wlList.length; i++){ %>
						    	<a class="nav-link my-0 py-1" id="0" href="data_wl?id=<%=i%>&chkAvg=0&select_id=<%=wlList[i]%>&start=<%=start%>&end=<%=end%>">
						            <i class="fas fa-tag fa-sm text-white-50"></i>
						            <%= wlList[i] %>
					            </a>
						    <%} %>
						    ...
						    <nav class="sb-sidenav-menu-nested nav accordion" id="sidenavAccordionPages">
						        ...
						        <div class="collapse" id="IPI-2" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                ...
						                <a class="nav-link my-0 py-1" id="<%=ipi2List[i]%>" href="data_ipi?id=0&select_id=<%=ipi2List[i]%>&chkAvg=&start=<%=start%>&end=<%=end%>"><%=ipi2List[i] %></a>
						                <% } %>
						            </nav>
						        ...
						        <div class="collapse" id="IPI-3" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                ...
						                <a class="nav-link my-0 py-1" id="<%=ipi3List[i]%>" href="data_ipi?id=1&select_id=<%=ipi3List[i]%>&chkAvg=&start=<%=start%>&end=<%=end%>"><%=ipi3List[i] %></a>
						                <% } %>
						            </nav>
						        </div>
						        ...
						        <div class="collapse" id="IPI-7" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                ...
						                <a class="nav-link my-0 py-1" id="<%=ipi7List[i]%>" href="data_ipi?id=2&select_id=<%=ipi7List[i]%>&chkAvg=&start=<%=start%>&end=<%=end%>"><%=ipi7List[i] %></a>
						                <% } %>
						            </nav>
						        </div>
						        ...
						        <div class="collapse" id="IPI-11" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                ...
						                <a class="nav-link my-0 py-1" id="<%=ipi11List[i]%>" href="data_ipi?id=3&select_id=<%=ipi11List[i]%>&chkAvg=&start=<%=start%>&end=<%=end%>"><%=ipi11List[i] %></a>
						                <% } %>
						            </nav>
						        </div>
						        ...
						        <div class="collapse" id="IPI-13" aria-labelledby="headingOne" data-parent="#sidenavAccordionPages">
						            <nav class="sb-sidenav-menu-nested nav">
						                ...
						                <a class="nav-link my-0 py-1" id="<%=ipi13List[i]%>" href="data_ipi?id=4&select_id=<%=ipi13List[i]%>&chkAvg=&start=<%=start%>&end=<%=end%>"><%=ipi13List[i] %></a>
						                <% } %>
						            </nav>
						        </div>
						    </nav>
		...
		<div id="layoutSidenav_content">
			<main>
				<div class="py-3 px-3 my-3 mx-3" style="width:1000px;">
					<div class="my-3 mx-3" style="width:1000px;">
						<h2>센서명(ipi낱개)<br></h2>
					</div>
					<div class="my-3">
						현장명 : ?<br>
	                   ...
	                   <script type= "text/javascript">
	                           ...
	                           function drawChart() {
	                               var data = google.visualization.arrayToDataTable([
	                                   ['심도', '4시간전','3시간전','2시간전','1시간전','현재'],
                                       ['대상심도', '4시간전데이터', '4시간전데이터', '4시간전데이터','4시간전데이터', '현재데이터']
	                               ]);
	                               var options = {
	                                   // legend: 'none',
	                                   legend: { position: "none", textStyle: {fontSize: 9}, maxLines: 2},
	                                   title: '지중경사계 그래프',
	                                   chartArea: {'width': '100%', 'height': '100%', top:80, bottom:80, left:70, right:40},
	                                   // animation: {startup: true,duration: 1000,easing: 'in' },
	                                   // selectionMode: 'multiple',
	                                   orientation: 'vertical',
	                                   tooltip: {trigger: 'selection'},
	                                   // aggregationTarget: 'category',
	                                   explorer: { axis: 'horizontal',maxZoomOut: 100 ,maxZoomIn: 1000 },
	                                   vAxis: {
	                                       baselineColor: 'black',
	                                       textStyle: {color: '#000', fontName: 'Arial', fontSize: 10},
	                                       showEveryText : 0,
	                                       gridlines: {minSpacing: '심도당 m(2)',},
	                                       direction : 1,
	                                       title:'depth (m)',
	                                       titleTextStyle: {color: '#000',fontName: 'Arial',italic:false, fontSize: 10},
	                                   },
	                                   hAxis: {
	                                       viewWindow: {max: 60, min: -60},
	                                       baselineColor: '#black',
	                                       textStyle: {color: '#000', fontName: 'Arial', fontSize: 10},
